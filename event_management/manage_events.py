...
from event_management.notify_users import send_reassign_notification, send_consultant_task_notification
...
def event_list(request):
    def doc_list(request):
        ...
        search_form = DocumentSearchForm(initial={'user': request.user})
        filters = []
        if (request.GET):
            search_form = DocumentSearchForm(request.GET, initial={'user': request.user})
            if (search_form.is_valid()):
                for each in search_form.changed_data:
                    if ('date' in each):
                        if ('uploaded_date_from' in each):
                            field_name = each.rsplit('_', 1)[0]
                            date_filter = field_name + "__gte"
                            filters.append(Q(**{date_filter: search_form.cleaned_data[each]}))
                            continue
                        if ('uploaded_date_to' in each):
                            field_name = each.rsplit('_', 1)[0]
                            date_filter = field_name + "__lte"
                            filters.append(Q(**{date_filter: search_form.cleaned_data[each]}))
                            continue
                        if ('next_revision_date_from' in each):
                            field_name = each.rsplit('_', 1)[0]
                            date_filter = field_name + "__gte"
                            filters.append(Q(**{date_filter: search_form.cleaned_data[each]}))
                            continue
                        if ('next_revision_date_to' in each):
                            field_name = each.rsplit('_', 1)[0]
                            date_filter = field_name + "__lte"
                            filters.append(Q(**{date_filter: search_form.cleaned_data[each]}))
                            continue
                    if ('document_code' in each):
                        filters.append(Q(**{'document_code__icontains': search_form.cleaned_data[each].upper()}))
                        continue
                    if (each == 'division'):
        ...
        searched_doc = 0
        if (len(filters) > 0):
            doc_list = Document.objects.filter(reduce(operator.and_, filters))
            total_doc_count = doc_list.count()
        else:
            doc_list = Document.objects.all().order_by('-uploaded_date')
            total_doc_count = doc_list.count()
        ...
        context = {'doc_list': doc_list, 'total_doc_count': total_doc_count}
